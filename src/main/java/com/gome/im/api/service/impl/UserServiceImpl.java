...
public class UserServiceImpl implements UserService {
	...
	public ResultModel<Object> saveUser(final User user) {
		...
		try {
			...
			String json = HttpUtil.httpRequest(HttpUtil.USER_REGISTER, JSON.toJSONString(req));
...
//			req.getReqUser().setImUserId(reqUser.getImUserId());
			String json = HttpUtil.httpGetRequest(HttpUtil.GET_USER_TOKEN_PATH, "&imUserId="+reqUser.getId());
			...
			if(reqUser.getUserName()!=null || reqUser.getNickName() != null){
				user.setNickName(reqUser.getNickName()==null?"":reqUser.getNickName());
				user.setUserName(reqUser.getUserName()==null?"":reqUser.getUserName());
				user.setAvatar(reqUser.getAvatar()==null?"":reqUser.getAvatar());
				user.setGender(reqUser.getGender());
				user.setRegion(reqUser.getRegion()==null?"":reqUser.getRegion());
				user.setAutograph(reqUser.getAutograph()==null?"":reqUser.getAutograph());
			}
			userMapper.updateUserInfo(user);
	...
	public ResultModel<Object> getUserInfo(User reqUser) {
		...
		try {
			User user = userMapper.getUserInfo(reqUser.getId());
			if(user == null){
				return  new ResultModel<Object>(ResultModel.RESULT_FAILURE,"获取信息失败,用户不存在",new HashMap<>());
			}
			user.setAppId(Global.APP_ID);
	...
	public ResultModel<Object> login(User reqUser,HttpServletRequest request) {
		...
		try {
			...
			if(user != null && user.getPassword().equals(reqUser.getPassword())){
				...
				user.setAppId(Global.APP_ID);
				if(System.currentTimeMillis() - user.getTokenValidity() >= 0){
					reqUser.setId(user.getId());
					user =(User) updateUserInfo(reqUser).getData();
				}
	...
	public HashMap<String,Object> getInfo(User user){
		...
		map.put("region",user.getRegion()==null?"":user.getRegion());
		map.put("birthday",user.getBirthday());
		map.put("autograph",user.getAutograph()==null?"":user.getAutograph());
